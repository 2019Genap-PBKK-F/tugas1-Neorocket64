...
//   name: 'jexcel',
  ...
  methods: {
    load() {
      axios.get('http://localhost:8024/api/periode/').then(res => {
        console.log(res.data)
        var options = {
          // url: 'http://localhost:3000/periode',
          data: res.data,
    ...
    changed(instance, cell, columns, row, value) {
      axios.get('http://localhost:8024/api/periode/').then(response => {
        var index = Object.values(response.data[row])
        index[columns] = value
        console.log('index selected: ' + index)
        axios
          .put('http://localhost:8024/api/periode/' + index[0], {
            nama: index[1],
            create_date: index[2],
            last_update: index[3]
          })
    ...
    insertrow(instance) {
      axios({
        method: 'post',
        url: 'http://localhost:8024/api/periode/',
        data: {
          nama: 'test',
    ...
    deleterow(instance, row) {
      axios.get('http://localhost:8024/api/periode/').then(response => {
        ...
        axios.delete('http://localhost:8024/api/periode/' + index[0])
      })
    }
  }
}
